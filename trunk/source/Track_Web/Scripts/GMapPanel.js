...
Ext.define('Ext.ux.GMapPanel', {
    ...
    */
    ...
    /**
    * Create a Single poligone.
    * @param {point} point where created a poligone
    * params {Object} poligonestyle an object defining the poligone style to use 
    */
    addPoligone: function (points, poligonstyle, marker) {
        //point, radio, 150, "#00FF00", 1, 0.3, "#00FF00", 0.3, idMovil
        var latConv = points.distanceFrom(new google.maps.LatLng(points.lat() + 0.1, points.lng())) / 100;
        var lngConv = points.distanceFrom(new google.maps.LatLng(points.lat(), points.lng() + 0.1)) / 100;

        //Loop
        var point = [];
        var step = parseInt(360 / poligonstyle.nodes) || 10;
        for (var i = 0; i <= 360; i += step) {
            var pint = new GLatLng(points.lat() + (radio / latConv * Math.cos(i * Math.PI / 180)), points.lng() +
                        (radio / lngConv * Math.sin(i * Math.PI / 180)));
            point.push(pint);
            bounds.extend(pint);
        }
        point.push(point[0]);
        ...
        //liWidth = liWidth || 2;
        poly = new google.maps.Polygon(points, poligonstyle.liColor, poligonstyle.liWidth, poligonstyle.liOpa, poligonstyle.fillColor, poligonstyle.fillOpa);
        ...
        //Ext.getCmp('thisMap' + idMovil).addOverlay(poly);
        Ext.getCmp('thisMap' + idMovil).getMap().addOverlay(poly);
    },
    /**
